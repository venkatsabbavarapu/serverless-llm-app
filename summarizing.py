...
def lambda_handler(event, context):
    ...
    output_bucket="clipsynopsis-output"
    ...
    try: 
        ...
        output_file_name=key.replace('.json','')
        s3_client.put_object(
            Bucket=output_bucket,
            Key=f'{output_file_name}-results.txt',
            Body=summary,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps(f"Successfully summarized {key} from bucket {input_bucket}. Summary: {summary}")
    }
